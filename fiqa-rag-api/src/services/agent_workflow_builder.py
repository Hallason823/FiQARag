...
class AgentWorkflowBuilder(LoggerMixIn):
    ...
    def retrieve_knowledge_node(self, state: FinancialAnalystState) -> FinancialAnalystState:
        ...
        search_limit = state.get("top_k", 5)
    ...
    def generate_answer_node(self, state: FinancialAnalystState) -> FinancialAnalystState:
        ...
        # Incorporação do histórico mantido pelo backend
        if history:
            full_context = (
                f"HISTÓRICO DA CONVERSA NESTA SESSÃO/TASK:\n{history}\n\n"
                f"DOCUMENTOS RECUPERADOS DA BASE FIQA:\n{context_data}"
            )
        else:
    ...
    def evaluate_evidence_routing(self, state: FinancialAnalystState) -> Literal["valid_evidence", "invalid_evidence"]:
        if state.get("score_maximo", 0.0) >= self._settings.evidence_threshold_score:
    ...
    def compile_agent_graph(self) -> Any:
        ...
        workflow_graph.add_conditional_edges(
            self.RETRIEVAL_STAGE_NODE_KEY,
            self.evaluate_evidence_routing,
            {"valid_evidence": self.CONTEXT_STAGE_NODE_KEY, "invalid_evidence": self.ABSTENTION_STAGE_NODE_KEY}
        )
